...
def api_temp():
    ...
    #tab_T=[24.64,25.89,27.89,65.34]


    if not ser:
        return jsonify({"error": "Série non disponible"}), 500

    # Réinitialisation des buffers série
    ser.reset_output_buffer()
    ser.reset_input_buffer()

    if request.method == 'POST':
        # Envoi de la commande au STM32
        ser.write(b'GET_T  ')
        # Lecture de la réponse
        tempo = ser.readline().decode().strip()
        print(f"Réponse brute reçue : {tempo}")  # Pour débogage
        # Extraction de la valeur numérique si le format est correct
        if tempo.startswith("T=") and "_C" in tempo:
            try:
                # Isolation de la valeur numérique (entre "T=" et "_C")
                numeric_value = float(tempo.split("=")[1].split("_")[0])
                tab_T.append(numeric_value)  # Ajout au tableau
                return jsonify(tab_T[-1])  # Retourne la dernière valeur ajoutée
            except ValueError:
                return jsonify({"error": "Impossible de convertir la valeur en nombre"}), 400
        else:
            return jsonify({"error": "Format de réponse invalide"}), 400

    elif request.method == 'GET':
        # Retourne toutes les valeurs collectées
        return jsonify(tab_T)
...
def api_pres():
    ...
    ser.reset_input_buffer()
    if request.method == 'POST':
        # Commande GET_P envoyée pour obtenir la pression (7 caractères espace espace)
        ser.write(b'GET_P  ')
        tempo = ser.readline().decode().strip()
        numeric_str = tempo.split("=")[1].split("_")[0]
        tab_P.append(numeric_str)
	#numeric_value = float(tempo.split("=")[1].split("Pa")[0])
        #tab_P.append(tempo)
